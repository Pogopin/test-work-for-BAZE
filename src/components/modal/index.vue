...
<form class="modal__body-form">
 ...
 <el-input
			...
			<div class="modal__body-input block__avatar">
				<label class="block-input-logo"><img class="block-input-img" src="@/assets/image/icon/star_.svg" alt="star-icon">Логотип</label>
				<div class="avatar-uploader" ref="refDropZone">
					<div class="avatar-uploader-img">
						<img v-if="inputData.imageUrl" :src="inputData.imageUrl" class="avatar" />
					</div>
					<div class="block__avatar-content" v-if="!inputData.imageUrl">
						<p class="block__avatar-content-title">Загрузите изображение или перетащите файл сюда</p>
						<p class="block__avatar-content-text">Максимальный размер изображения 10 Мб.<br>Допустимые форматы: .jpeg .png .webp .svg</p>

						<label class="block__avatar-content-input">
							<!-- <input type="file" name="avatar-image" ref="inputt">			@change="onInputChange"				 -->
							<input type="file" name="avatar-image" @change="onInputChange"	>
							<span><img src="@/assets/image/icon/plus_16.svg" alt="icon-plus">Загрузить</span>
						</label>		
					</div>					
				</div>				
			</div>
		...
		<div class="modal__bottom" v-if="inpDataForm && isTrue">
			<Block3
				:data="inpDataForm"
				@update:isVisible="isVisibleBlock = true; isTrue = false"
			/>
		</div>
	</div>
...
.modal__header, .modal__body, .modal__bottom {
	background-color: #FFF;
	border-radius: 16px;
	padding: 24px;
	color: #353535;
}
.modal__header-title, .modal__body-top-org {
...
.block-input-type, .block-input-name, .block-input-description, .block-input-logo {
	color:#0A0A0A;
	font-weight: 500;
	font-size: 14px;
	line-height: 12px;
